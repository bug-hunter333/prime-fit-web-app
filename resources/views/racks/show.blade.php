...
<div class="hidden md:block bg-black text-white p-4">
    ...
    <main class="p-6 max-w-7xl mx-auto animate-fade-in">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <!-- Product Image -->
            <img src="{{ $rack->image_url ?? asset('assets/dumbell_10_kg-removebg-preview.png') }}"
                 alt="{{ $rack->name }}"
                 class="rounded-xl shadow-lg w-full object-contain max-h-[500px]" />

            <!-- Product Info -->
            <div>
                <p class="text-sm text-gray-400 mb-1">Product ID: {{ $rack->id }}</p>
                <h1 class="text-4xl text-green-500 font-bold mb-2">{{ $rack->name }}</h1>
                <p class="text-3xl text-green-400 font-bold mb-4">${{ number_format($rack->price, 2) }}</p>

                @if($rack->weight > 0)
                    <p class="mb-2"><span class="text-green-400">Weight: {{ $rack->weight }}kg</span></p>
                @endif

                <p class="mb-4">
                    <span class="text-green-400">Stock:</span> 
                    <span class="font-semibold text-white">
                        {{ $rack->quantity }} available
                        @if($rack->quantity == 0)
                            <span class="text-red-400 ml-2">(Out of Stock)</span>
                        @elseif($rack->quantity < 5)
                ...
                <div class="mb-4">
                    ...
                    <input id="quantity" type="number" name="quantity" min="1" max="{{ $rack->quantity }}"
                           value="1" class="w-24 px-3 py-2 bg-gray-700 rounded-md text-white focus:outline-none"
                           @if($rack->quantity == 0) disabled @endif>
                </div>

                <!-- Buttons -->
                <form action="{{ route('racks.add') }}" method="POST" class="inline">
                  @csrf
                  <input type="hidden" name="product_id" value="{{ $rack->id }}">
                  ...
                  <button type="submit"
                          ...
                          @if($rack->quantity == 0) disabled @endif>
                      {{ $rack->quantity == 0 ? 'Out of Stock' : 'Add to Cart' }}
                  </button>
              </form>

              <!-- Buy Now Button -->
              {{-- <form action="{{ route('racks.checkout') }}" method="POST" class="inline"> --}}
                  {{-- @csrf
                  <input type="hidden" name="product_id" value="{{ $rack->id }}">
                  <input type="hidden" name="quantity" value="1">
                  ...
                  <button type="submit"
                          class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md font-bold transition
                                disabled:bg-gray-500 disabled:cursor-not-allowed"
                          @if($rack->quantity == 0) disabled @endif>
                      {{ $rack->quantity == 0 ? 'Out of Stock' : 'Buy Now' }}
                  </button> --}}
              </form>
            </div>
            </div>
...
<div class="mt-12">
  ...
  <div id="specs" class="tab-content mt-6 hidden animate-fade-in">
    <div class="bg-gray-800 p-5 rounded-lg shadow-lg transition duration-300 ease-in-out">
      ...
      <ul class="space-y-2 text-gray-300">
        <li><span class="text-gray-400">ID:</span> {{ $rack->id }}</li>
        <li><span class="text-gray-400">Name:</span> {{ $rack->name }}</li>
        <li><span class="text-gray-400">Price:</span> ${{ number_format($rack->price, 2) }}</li>
        <li><span class="text-gray-400">Weight:</span> {{ $rack->weight }}kg</li>
        <li><span class="text-gray-400">Stock:</span> {{ $rack->quantity }}</li>
        <li>
          <span class="text-gray-400">Status:</span> 
          <span class="{{ $rack->quantity > 0 ? 'text-green-400' : 'text-red-400' }}">
            {{ $rack->quantity > 0 ? 'In Stock' : 'Out of Stock' }}
          </span>
        </li>
      </ul>
    </div>
  ...
  <div id="faqs" class="tab-content mt-6 hidden animate-fade-in">
    <div class="bg-gray-800 p-5 rounded-lg shadow-lg transition duration-300 ease-in-out">
      ...
      @if(!empty($rack->faq))
      <p class="mt-3 text-gray-400 italic">{{ $rack->faq }}</p>
      @endif
    </div>
  </div>
</div>
