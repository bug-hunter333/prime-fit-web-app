...
<div class="hidden md:block bg-black text-white p-4">
    ...
    <main class="p-6 max-w-7xl mx-auto animate-fade-in">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <!-- Product Image -->
            <img src="{{ $barbell->image_url ?? asset('assets/dumbell_10_kg-removebg-preview.png') }}"
                 alt="{{ $barbell->name }}"
                 class="rounded-xl shadow-lg w-full object-contain max-h-[500px]" />

            <!-- Product Info -->
            <div>
                <p class="text-sm text-gray-400 mb-1">Product ID: {{ $barbell->id }}</p>
                <h1 class="text-4xl text-green-500 font-bold mb-2">{{ $barbell->name }}</h1>
                <p class="text-3xl text-green-400 font-bold mb-4">${{ number_format($barbell->price, 2) }}</p>

                @if($barbell->weight > 0)
                    <p class="mb-2"><span class="text-green-400">Weight: {{ $barbell->weight }}kg</span></p>
                @endif

                <p class="mb-4">
                    <span class="text-green-400">Stock:</span> 
                    <span class="font-semibold text-white">
                        {{ $barbell->quantity }} available
                        @if($barbell->quantity == 0)
                            <span class="text-red-400 ml-2">(Out of Stock)</span>
                        @elseif($barbell->quantity < 5)
                ...
                <div class="mb-4">
                    ...
                    <input id="quantity" type="number" name="quantity" min="1" max="{{ $barbell->quantity }}"
                           value="1" class="w-24 px-3 py-2 bg-gray-700 rounded-md text-white focus:outline-none"
                           @if($barbell->quantity == 0) disabled @endif>
                </div>

            <!-- Add to Cart Button -->
                <form action="{{ route('barbells.add') }}" method="POST" class="inline">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $barbell->id }}">
                    ...
                    <button type="submit"
                        ...
                        @if($barbell->quantity == 0) disabled @endif>
                        {{ $barbell->quantity == 0 ? 'Out of Stock' : 'Add to Cart' }}
                    </button>
                </form>
            </div>
        </div>
...
<div class="mt-12">
  ...
  <div id="specs" class="tab-content mt-6 hidden animate-fade-in">
    <div class="bg-gray-800 p-5 rounded-lg shadow-lg transition duration-300 ease-in-out">
      ...
      <ul class="space-y-2 text-gray-300">
        <li><span class="text-gray-400">ID:</span> {{ $barbell->id }}</li>
        <li><span class="text-gray-400">Name:</span> {{ $barbell->name }}</li>
        <li><span class="text-gray-400">Price:</span> ${{ number_format($barbell->price, 2) }}</li>
        <li><span class="text-gray-400">Weight:</span> {{ $barbell->weight }}kg</li>
        <li><span class="text-gray-400">Stock:</span> {{ $barbell->quantity }}</li>
        <li>
          <span class="text-gray-400">Status:</span> 
          <span class="{{ $barbell->quantity > 0 ? 'text-green-400' : 'text-red-400' }}">
            {{ $barbell->quantity > 0 ? 'In Stock' : 'Out of Stock' }}
          </span>
        </li>
      </ul>
    </div>
  ...
  <div id="faqs" class="tab-content mt-6 hidden animate-fade-in">
    <div class="bg-gray-800 p-5 rounded-lg shadow-lg transition duration-300 ease-in-out">
      ...
      @if(!empty($barbell->faq))
      <p class="mt-3 text-gray-400 italic">{{ $barbell->faq }}</p>
      @endif
    </div>
  </div>
</div>
